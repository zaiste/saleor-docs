...
Use cases:
- store additional shipping note to the order in `Order` metadata
- add special flag to the product (for example `dont-synchronize`) so your custom stock synchronization App would omit some Products 
...
Models which implement this interface have two additional fields:
- `metadata`: public and visible to unauthenticated users
- `privateMetadata`: protected and visible only to staff users, plugins, and Apps with appropriate permissions
